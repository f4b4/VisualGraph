...
static ogdf::Color g_emptyColor(255, 255, 255, 255);
...
void GraphPainter::SetGraphAttributes(const std::shared_ptr<ogdf::GraphAttributes> &graphAttributes)
{
    ...
    for_each(m_graphAttributes->constGraph().firstNode(), [&](const ogdf::node &item)
    {
        ...
        V4 color = m_defaultNodeColor;
        if (g_emptyColor != ogdfColor)
        {
            color = V4(
                ogdfColor.red() / 255.f,
    ...
    gl::BufferData(gl::GL_ARRAY_BUFFER, sizeof(Vertex) * m_nodesVertexCount,
                   nodesVectorVec.data(), gl::GL_STATIC_DRAW);
    ...
    gl::BufferData(gl::GL_ELEMENT_ARRAY_BUFFER,
                   sizeof(uint) * m_nodesIndexCount, nodesIndexVec.data(), gl::GL_STATIC_DRAW);
    gl::BindBuffer(gl::GL_ELEMENT_ARRAY_BUFFER, 0);

    gl::GenBuffers(1, &m_nodesBorderIndexBuffer);
    gl::BindBuffer(gl::GL_ELEMENT_ARRAY_BUFFER, m_nodesBorderIndexBuffer);
    gl::BufferData(gl::GL_ELEMENT_ARRAY_BUFFER, sizeof(uint) * m_nodesBorderIndexCount,
                   nodesBorderIndexVec.data(), gl::GL_STATIC_DRAW);
    gl::BindBuffer(gl::GL_ELEMENT_ARRAY_BUFFER, 0);
...
// edges
    ...
    gl::BufferData(gl::GL_ARRAY_BUFFER, sizeof(float) * linesVertexVec.size(),
                   linesVertexVec.data(), gl::GL_STATIC_DRAW);
    gl::BindBuffer(gl::GL_ARRAY_BUFFER, 0);
}
